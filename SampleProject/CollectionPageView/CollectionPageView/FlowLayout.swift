...
final class FlowLayout: UICollectionViewFlowLayout {
    ...
    override func targetContentOffset(forProposedContentOffset proposedContentOffset: CGPoint, withScrollingVelocity velocity: CGPoint) -> CGPoint {
        ...
        if velocity.x == 0 {
            // スワイプせずに指を離した場合、画面中央から一番近い要素を取得
        ...
        let cellLeftMagin = (collectionView.bounds.width - attributes.bounds.width) * 0.5
        return CGPoint(x: attributes.frame.minX - cellLeftMagin, y: collectionView.contentOffset.y)
    }

    // 画面中央に一番近いセルのattributesを取得
    ...
    // 1ページずつページングさせるために、あらかじめ表示されている attributes の配列を取得
